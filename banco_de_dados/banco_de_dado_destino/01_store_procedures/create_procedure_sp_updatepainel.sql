...
ALTER PROCEDURE [dbo].[sp_updatePainel]
    @idPasta            INT,                    --Id da idpasta.
	@ItemID             NVARCHAR(40),           -- Id interno do objeto.
	@Objeto             nvarchar(425),          -- Novo nome do objeto.
	...
	@Tamanho            FLOAT,                  -- Novo tamanho
	@CreatedByUserName  nvarchar(260),          -- Usuário que criou o objeto.
	@ModifiedByUserName nvarchar(260)           -- Usuário que modificou o objeto.
AS
BEGIN
	--Controle de erro.
	BEGIN TRANSACTION;  
		...
		BEGIN TRY  
            ...
            SELECT 'Objeto'     --TabName
                ...
                , 'UPDATE'      --acao
                , CAST(A.[idpainel] AS nvarchar(10))+' ; '+A.[ItemID]+' ; '+A.[ParentID]+' ; '+A.[painel]+' ; '+A.[Localizacao]+' ; '+CAST(A.[DataDaCriacao] AS NVARCHAR(24))+' ; '+CAST(A.[DataDaModificacao] AS NVARCHAR(24))+' ; '+ CAST(A.[DiasSemAlteracao] AS NVARCHAR(3)) -- valorAnterior
                , CAST(A.[idpainel] AS nvarchar(10))+' ; '+A.[ItemID]+' ; '+A.[ParentID]+' ; '+ @Objeto  +' ; '+@Localizacao   +' ; '+CAST(@DataDaCriacao AS NVARCHAR(24))+   ' ; '+CAST(@DataDaModificacao AS NVARCHAR(24))   +' ; '+ CAST(@DiasSemAlteracao AS NVARCHAR(3)) -- valorDepois
            FROM [objects].[painel] AS A
            WHERE A.[idPasta] = @idPasta AND A.[ItemID] = @ItemID

		UPDATE [objects].[painel]
		   SET [painel]             = @Objeto 
